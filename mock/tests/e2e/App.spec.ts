...
test("i can load a CSV successfully", async ({ page }) => {
  ...
  const responseText = await page.evaluate(() => {
    const responseElement = document.querySelector("load_file"); // Replace "#responseElementSelector"
...
test("i cannot load a CSV if my input isn't well formed", async ({ page }) => {
  ...
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file load_file fakedata.csv");
